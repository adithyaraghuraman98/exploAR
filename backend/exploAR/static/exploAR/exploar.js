  ...
  if (clicked) {
    ...
    if (intersects.length > 0) {
      ...
      if (obj instanceof THREE.Sprite & obj.name != "distance") {
        window.location.href = ('https://exploar.us/place-info?place_id='+places.get(obj).place_id);
      }
    }
    clicked = false;
  }
...
function setPlaceType(type) {
  ...
  var dropdowns = document.getElementsByClassName("material-icons");
  var i;
  for (i = 0; i < dropdowns.length; i++) {
    var openDropdown = dropdowns[i];
    if (openDropdown.classList.contains('icon-activated')) {
      openDropdown.classList.remove('icon-activated');
    }
  }

  document.getElementById("place-id").value = "";
  document.getElementById(type).classList.add("icon-activated");
  getLocation();
}
  ...
  if (position) {
      ...
      if (document.getElementById("place-id").value == "") {
        ...
        $.ajax({
            url: "/get-places-json",
            type: "GET",
            data: "lat="+latDeg+"&lng="+longDeg+"&category="+placeType,
            dataType : "json",
...
function placeObject(fields, latitude, longitude) {
  ...
  var cubeGeometry = new THREE.OctahedronGeometry(1, 1, 1);
  var cubeMaterial = new THREE.MeshNormalMaterial({ wireframe: true });
  cubeMaterial.transparent = true;

  var cube = new THREE.Mesh(cubeGeometry, cubeMaterial);
  cube.name = 'cube';
  cube.position.set(xCube, yCube, zCube);
  ...
  sprite = makeTextSprite(" "+fields["name"], {"backgroundColor": { r:255, g:255, b:255, a:opacity }});
  sprite.name = fields["name"];
  sprite.position.set(xCube, yCube, zCube);
  places.set(sprite, { place_id: fields["google_place_id"], lat: lat, lng: lng});
  scene.add(sprite);
